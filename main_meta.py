...
train_dataset_names = ["hebrew_chars", "fashion_mnist", "kmnist", "math_shapes"]#, "math_shapes"]
...
def soft_clustering_loss(embeddings, centroids, temperature=1.0, alpha=1.0):
    
    dists = pairwise_squared_distances(embeddings, centroids)
    
    assignments = F.softmax(-dists / temperature, dim=1)  # (N, K)
    
    clustering_loss = torch.mean(torch.sum(assignments * dists, dim=1))
    
    p_bar = assignments.mean(dim=0)  # (K,)
    
    # Higher entropy means points are distributed evenly across clusters
    entropy_reg = -torch.sum(p_bar * torch.log(p_bar + 1e-8))
    
    # Total Inner Loss: Minimize clustering error AND Maximize entropy (Negative sign)
...
def soft_clustering_loss(embeddings, centroids, temperature=0.5, alpha=1.0):
    ...
    # 1. Cosine Similarity (since inputs are normalized)
    # Range: [-1, 1]
    sim = torch.matmul(embeddings, centroids.T)
    
    # 2. Convert to "Distance" for softmax
    # We want max similarity -> max prob.
    # Softmax(sim / temp) works directly.
    # No negative sign needed if using similarity. 
    # If using distance: dist = 2 - 2*sim. 
    # Let's use similarity directly for numerical stability.
    
    assignments = F.softmax(sim / temperature, dim=1) # (N, K)
    
    # 3. Clustering Loss: Maximize similarity to assigned cluster
    # Minimize: 1 - weighted_similarity
    weighted_sim = torch.sum(assignments * sim, dim=1)
    clustering_loss = torch.mean(1.0 - weighted_sim)
    
    # 4. Entropy Regularization (prevent all points going to 1 centroid)
    p_bar = assignments.mean(dim=0) # (K,)
    entropy_reg = -torch.sum(p_bar * torch.log(p_bar + 1e-8))
    
    # Minimize clustering error, Maximize entropy (minimize negative entropy)
    total_loss = clustering_loss - (alpha * entropy_reg)
