...
def eiv(element_or_vector: Union[list, float], index: int):
    try:
...
class VAE(nn.Module):
    ...
    def __init__(self, w: int, h: int, ls_dim: int, in_channels: int = 3):
        ...
        self.encoder = nn.Sequential(
            ResidualBlock(
                in_channels=1, out_channels=32, kernel_size=3, padding=1, strides=(1, 2)
            ),  # 28 -> 14
            ResidualBlock(
                in_channels=32,
                out_channels=64,
...
def calc_cnn_transform_size(
    x: int, padding: int, kernel: int, stride: int, dilation: int = 1
):
